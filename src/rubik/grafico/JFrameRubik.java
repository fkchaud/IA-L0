...
public class JFrameRubik extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jPanelRubik.setFont(new java.awt.Font("Tahoma", 0, 12)); // NOI18N
        ...
        SeleccionEstrategia.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Búsqueda Anchura en Árbol", "Búsqueda Costo Uniforme en Árbol", "Búsqueda Profundidad en Árbol", "Búsqueda Anchura en Grafo", "Búsqueda Costo Uniforme en Grafo", "Búsqueda Profundidad en Grafo", "Búsqueda Profundidad Limitada en Grafo", "Búsqueda Profundidad Iterativa en Grafo" }));
...
private void btAplicarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btAplicarActionPerformed
    ...
    /**
     * Desordena el cubo y devuelve la lista de pasos.
     * Toma la semilla ingresada en la interfaz, la settea en el cubo, y realiza
     * los movimientos con el método {@link rubik.modelo.Cubo#mezclar(int)}.
     * Luego concatena dichos movimientos en un String, y lo devuelve.
     * @param pasos Cantidad de pasos en que se va a mover el cubo.
     * @return Cadena concatenada de pasos en los que se movió el cubo.
     */
    private String desordenarCubo(int pasos) {                                  // Recibe la cantidad de pasos para desordenar
        Cubo c = jPanelRubik.getCubo();                                         // Obtiene el cubo
        int semilla = Integer.parseInt( selecionSemilla.getText() );            // Obtiene la semilla de la interfaz
        c.rnd.setSeed(semilla);                                                 // Settea la semilla en el cubo
        Vector<Movimiento> movimientos = c.mezclar(pasos);                      // Mezcla el cubo con dicha cantidad de pasos, y
                                                                                // devuelve los movimientos realizados
        jPanelRubik.setCubo(c);                                                 // Guarda el cubo 
        String strMovimientos = ""; 
        for (Movimiento m : movimientos) {
            strMovimientos = strMovimientos + m.toString() + " ";               // Arma un string con los movimientos realizados
        }
        return (strMovimientos);                                                // Devuelve el string de movimientos
    }

    /**
     * Mueve el cubo en una serie de movimientos.
     * Ejecuta los movimientos indicados en <code>text</code>, mostrándolos en
     * la animación con el método {@link rubik.grafico.JPanelRubik#animarCubo(java.util.Vector)}.
     * @param text Lista de movimientos, separados por un espacio, que se desea
     *             que realice el cubo. No distingue mayúsculas y minúsculas.
     */
    private void moverCubo(String text) {                                       
        if (text != null) {                                                     // Si el texto no está vacío... (si está vacío no hace nada)
            Vector<Movimiento> movimientos = new Vector<Movimiento>();          // Inicializa una lista de movimientos vacía
            String[] componentes = text.split(" ");                             // Separa los componentes del string en una lista de strings
            for (String etq : componentes) {                                    // Para cada componente de la lista...
                // Burca id del movimiento
                for (int i = 0; i < Movimiento.etq_corta.length; i++) {         
                    if (etq.equalsIgnoreCase(Movimiento.etq_corta[i])) {        // ... lo compara con cada movimiento ...
                        movimientos.add(Movimiento.movimientosPosibles[i]);     // ... y si coinciden, lo transforma en objeto Movimiento.
                    }
                }
            }
            if (movimientos.size() > 0) {                                       // Si hubo movimientos...
                jPanelRubik.animarCubo(movimientos);                            // ... anima el cubo para que realice esos movimientos.
            }
        }
    }
